...
def determine_cost_matrix(dets1, dets2, max_dist):
    ...
    for i, det_i in enumerate(dets1):
        ...
        for j in range(dets2.shape[0], dets2.shape[0] + dets1.shape[0]):
            matrix[i, j] = max_dist

    return matrix


def munkres_tracking(dets, max_dist=40):
    frames = np.unique(dets[:, 3])
    frame_ids = np.array(range(1, dets[dets[:, 3] == frames[0]].shape[0] + 1, 1))
    ids_list = list(frame_ids)
    for i, frame in enumerate(frames[:-1]):
        print('frame', frame)
        frame_dets = dets[dets[:, 3] == frame]
        next_frame_dets = dets[dets[:, 3] == frames[i + 1]]
...
if __name__ == '__main__':
    file = '../../output/test/detections.csv'
    # print(os.listdir(file))
    detections = pd.read_csv(file, index_col=False)
    print(detections)
    trajectories = munkres_tracking(detections.to_numpy())
