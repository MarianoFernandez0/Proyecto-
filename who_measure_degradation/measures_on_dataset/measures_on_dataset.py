...
if __name__ == "__main__":
    indir = 'dataset_tracks'
    out_base = 'measures'
    res = 1
    fps = 0
    datasets = sorted([directory for directory in os.listdir(indir) if os.path.isdir(os.path.join(indir, directory))])
    for dataset in datasets:
        ...
        for result in result_files:
            ...
            fps = float((result.split("Hz")[0]).split("(")[-1])
