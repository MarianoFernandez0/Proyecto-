...
with TiffWriter('images/segmentation_stack.tif', bigtiff=True) as tif:
    for frame in range(input_sequence.shape[0]):
        tif.save(segmentation_stack[frame, :, :], photometric='minisblack',
                 resolution=(input_sequence.shape[1], input_sequence.shape[2]))
...
for image in range(input_sequence.shape[0]):
    for i in range(input_sequence.shape[1]):
        for j in range(input_sequence.shape[2]):
            ...
            elif (segmentation_stack[image, i, j] == ground_truth[image, i, j]) and segmentation_stack[
                image, i, j] == 255:
...
with TiffWriter('images/matching_table.tif', bigtiff=True) as tif:
    for frame in range(input_sequence.shape[0]):
        tif.save(matching_table[frame, :, :], photometric='minisblack',
                 resolution=(input_sequence.shape[1], input_sequence.shape[2]))
